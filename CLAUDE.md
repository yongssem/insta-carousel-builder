...
### ⭐ 기본 산출 형태 = 정적 이미지 캐러셀 (v5)

캐러셀 요청이 오면 **기본으로 PNG 만** 만듭니다. `--skip-video` 를 붙이세요.

```bash
node scripts/make-carousel.mjs --data <json> --topic <t> --skip-video
```

> **왜 모션을 뺐나** — 한동안 기본이 모션이었는데 **식상해졌습니다.**
> 처음엔 눈에 띄었지만 계정 전체가 같은 방식이면 그게 곧 배경이 됩니다.
> 모션은 **그 장에 움직여야 할 이유가 있을 때만** 씁니다
> (숫자가 올라가는 장면, Before→After 전환 등).
> 그때만 `--motion 2,5` 처럼 장을 직접 지정하세요.
...
한 번에 PNG 본문 + 엔드카드 + `output/<topic>/upload/` 조립까지 끝납니다.
(`--skip-video` 없이 돌리면 MP4 도 같이 만듭니다 — 지금은 기본이 아닙니다.)
**단계별로 따로 돌리지 마세요** — 재인코딩이 끝나기 전에 이전 산출물을 묶는
사고가 실제로 났습니다. 이 스크립트는 전부 동기 실행 후 최신성까지 검증합니다.

업로드 폴더 구성(기본): `01-cover.png` ~ `06.png` + `07-endcard.png` — **전부 PNG**

#### 💾 로컬 다운로드 폴더에 자동 저장

파이프라인이 끝나면 업로드 세트를 **다운로드 폴더의 날짜 폴더**에 그대로 복사합니다.

```
~/Downloads/2026-08-05_<topic>/
├── 01-cover.png ~ 09.mp4 ~ 10-endcard.png   # upload/ 전체
├── caption.md                                # 있으면 같이
├── news.md                                   # 3way 산출물도 같이
└── blog.md
```

- `~/Downloads` 가 없으면 `~/다운로드` 를 찾고, 둘 다 없으면 조용히 건너뜁니다
- `--out-dir <경로>` — 날짜 폴더 없이 지정한 위치에 바로 저장
- `--no-downloads` — 자동 복사 끄기

⚠️ **작업 환경이 원격(클라우드 세션)이면 그 컨테이너의 홈으로 저장됩니다.**
선생님 PC 다운로드 폴더에 떨어뜨리려면 **로컬에서 이 명령을 실행**해야 합니다.
...
| 값 | 대상 | 언제 |
|:---|:---|:---|
| `alt` | 짝수 장 (2,4,6) | 정적/모션 교차로 넘길 때 리듬이 생김 |
| `all` | 본문 2~9 전부 | 전 장에 데이터 시각화가 있을 때 |
| `data` | 시각 요소가 있는 장만 | 데이터 중심 편 |
| `last` | 마지막 장(Outro)만 | 저장 CTA에만 힘 주고 싶을 때 |
| `2,5,8` | 번호 직접 | 후킹·반전·클라이맥스만 |
| ⭐ `none` (**기본**) | 없음 | 정적 캐러셀. `--skip-video` 와 같음 |
...
### 🎬 움직이는 캐러셀 (모션 슬라이드) — 지금은 기본이 아닙니다

> 아래는 **모션이 꼭 필요한 편에서만** 참고하세요. 기본은 정적 PNG 입니다.
...
## ⭐ 기본 산출물 — 본문 6장 + 엔드카드 1장 = 7장

**기본은 6장입니다.** 예전 기본값은 9장이었는데 대부분의 편에서 길었습니다.

- **slide-01~06 (본문)**: Cover 1장 + 본문 4장 + Outro 1장
- **마지막 장 (엔드카드)**: 책 홍보 고정 페이지 — **모든 캐러셀에 항상 붙임**
  - 번호는 **본문 다음**입니다. 본문 6장이면 `slide-07.png`
  - `node scripts/make-endcard.js --topic <topic> --n 7`
  - `make-carousel.mjs` 가 본문 장수를 세서 자동으로 맞춥니다 (직접 지정 불필요)

**왜 6장인가** (2026 실측 기준): 7~10장이 완독률·체류시간 스위트스팟이고,
12장을 넘으면 완독률이 약 40% 떨어집니다.
**본문 6 + 엔드카드 1 = 7장**이 그 구간의 하단이라, 완독률을 가장 높게 가져갑니다.

### 9장으로 갈 때

**구조가 요구할 때만** 늘립니다. 습관으로 늘리지 마세요.

| 6장 (기본) | 9장 |
|:---|:---|
| 소개 · 통념 깨기 · 비교 · 소식 | 단계별 튜토리얼 (순서를 빠뜨리면 못 따라 함) |
| 한 가지 메시지 | 체크리스트 (항목이 실제로 7개 이상) |

> 판단 기준 — **한 장을 빼면 이해가 안 되는가.**
> 빼도 되면 6장이 맞습니다. "할 말이 많아서"는 늘릴 이유가 아닙니다.

엔드카드를 Outro와 **교체하지 말 것** — 저장 CTA가 사라져 저장률이 떨어집니다.

그 외 개수는 사용자가 명시한 경우만 수용.
...
구독료 변경 · 새 모델 출시 · 새 기능. 주요 툴은 **클로드(코드) · 러버블 · Codex · Gemini**.
정책 뉴스보다 가볍고, 매주 반드시 뭔가 나오고, 독자가 바로 써먹습니다.
⚠️ 요금 정보는 국가·시점별로 다르므로 **공식 요금 페이지 + 확인 날짜**를 반드시 병기.

### 3way 배포 — 한 소재로 뉴스·캐러셀·블로그

```bash
/three-way <소재>   # 뉴스 → 캐러셀 → 블로그 순으로 생산
```

`knowledge/patterns/3way-repurpose.md` 가 규칙입니다. 핵심만:
...
**복붙하면 셋 다 죽습니다.** 제목·첫문장·마무리는 반드시 다르게 씁니다.
**블로그를 먼저 쓰지 마세요** — 직접 며칠 써보고 써야 뉴스 요약이 안 됩니다.
